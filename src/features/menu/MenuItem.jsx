import { Link } from 'react-router-dom';
import { formatCurrency } from './../utils/helpers';
import Button from './../ui/Button';
import { useDispatch, useSelector } from 'react-redux';
import { addItem, deleteItem } from '../cart/cartSlice'
import { getCurrentQuantity } from './../cart/cartSlice';
import UpdateItemQuantity from './../cart/UpdateItemQuantity';


function MenuItem({ pizza }) {


  const dispatch = useDispatch()
  const { id, name, unitPrice, ingredients, soldOut, imageUrl } = pizza;

  const curQuantity = useSelector(getCurrentQuantity(id))
  const isInCart = curQuantity > 0




  function handleAddItem() {

    const newItem = {
      pizzaId: id,
      name,
      qunatity: 1,
      unitPrice,
      ingredients,
      totalPrice: unitPrice * 1,
      soldOut,
      imageUrl,
    }
    dispatch(addItem(newItem))

  }




  return (



    <li className='flex gap-4 py-2 '>
      <img src={imageUrl} alt={name} className={`h-24 ${soldOut ? 'opacity-70 grayscale' : ''}`} />
      <div className='flex flex-col grow'>
        <p className='font-medium'>{name}</p>
        <p className='text-sm italic text-stone-500 capitalize'>{ingredients.join(', ')}</p>
        <div className='mt-auto flex items-center justify-between'>
          {!soldOut ? <p className='text-sm'>{formatCurrency(unitPrice)}</p> : <p className='text-sm uppercase font-mediume'>Sold out</p>}
        
          {isInCart &&<div className='flex gap-10'>
            <UpdateItemQuantity pizzaId={id} curQuantity={curQuantity} />
            <Button onclick={() => { dispatch(deleteItem(id)) }} type={'small'}>delete</Button>
          </div> }
          {!soldOut && !isInCart && <Button onclick={handleAddItem} type='small'>Add To Cart</Button>}


        </div>
      </div>
    </li>

  );
}

export default MenuItem;
